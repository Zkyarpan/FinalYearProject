...
const StoryCreatePage = () => {
  ...
  const { _id: userId, isAuthenticated } = useUserStore(); // Get current user info
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (formData.imageFile) {
        formDataToSend.append('storyImageFile', formData.imageFile);
      }
      ...
      if (!response.ok) {
        throw new Error(
          result.ErrorMessage?.[0]?.message || 'Failed to create story'
        );
      }
      ...
      <div className="max-w-[1000px] mx-auto px-4">
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-muted-foreground mt-1">
              Share your mental health journey with the community
            </p>
          </div>
        ...
        {!isAuthenticated && (
          <div className="bg-yellow-100 dark:bg-yellow-900 p-3 rounded-md mb-6 text-yellow-800 dark:text-yellow-200">
            <p>
              You must be logged in to publish a story. Please log in first.
            </p>
          </div>
        )}
