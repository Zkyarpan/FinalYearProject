...
const WORD_LIST = [
  'FOCUS', 'BRAIN', 'RELAX', 'PEACE', 'SLEEP', 'HAPPY', 'CALM', 'AWARE',
  'BREATH', 'ALERT', 'CLEAR', 'QUIET', 'SMILE', 'TRUST', 'FRESH', 'VITAL',
  'SOUND', 'HEART', 'LIGHT', 'DREAM', 'POWER', 'SHINE', 'THINK', 'BLOOM',
  'GROW', 'SENSE', 'VIBE', 'ZEAL', 'MIND', 'AURA', 'FLOW', 'GLOW'
];
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="bg-gradient-to-br from-gray-900 to-gray-950 border-gray-800 text-white max-w-md mx-auto">
        ...
        <div className="mb-4">
          <p className="text-sm text-center mb-2 text-gray-400">Word Distribution</p>
              ...
              <div key={index} className="flex items-center gap-2">
                ...
                <div className="flex-1 h-6 rounded-sm overflow-hidden bg-gray-800 relative">
                  <div 
                    ...
                    style={{ width: `${(count / Math.max(...gameData.wordDistribution, 1)) * 100}%` }}
                  ></div>
                  <span className="absolute right-2 top-1/2 transform -translate-y-1/2 text-xs text-white">{count}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 flex items-center justify-center font-bold bg-emerald-600 rounded">R</div>
              <span>Correct letter and position</span>
            </div>
            
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 flex items-center justify-center font-bold bg-amber-500 rounded">I</div>
              <span>Correct letter, wrong position</span>
            </div>
            
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 flex items-center justify-center font-bold bg-gray-700 rounded">G</div>
          ...
          <div>
            ...
            <div className="flex gap-1 mt-2">
              <div className="w-8 h-8 flex items-center justify-center font-bold bg-emerald-600 rounded">P</div>
              <div className="w-8 h-8 flex items-center justify-center font-bold bg-gray-700 rounded">I</div>
              <div className="w-8 h-8 flex items-center justify-center font-bold bg-gray-700 rounded">X</div>
              <div className="w-8 h-8 flex items-center justify-center font-bold bg-amber-500 rounded">E</div>
              <div className="w-8 h-8 flex items-center justify-center font-bold bg-gray-700 rounded">L</div>
            </div>
            <p className="mt-2 text-sm text-gray-400">
              'P' is in the correct spot, 'E' is in the word but in the wrong spot,
              and the other letters are not in the word.
            </p>
          </div>
        </div>
                ...
                <div key={index} className="flex items-center gap-2">
                  ...
                  <div className="flex-1 h-6 rounded-sm overflow-hidden bg-gray-800 relative">
                    <div 
                      ...
                      style={{ width: `${(count / Math.max(...gameStats.guessDistribution, 1)) * 100}%` }}
                    ></div>
                    <span className="absolute right-2 top-1/2 transform -translate-y-1/2 text-xs text-white">{count}</span>
                  </div>
                </div>
...
const WordleGame = ({ onGameComplete }) => {
  ...
  const getLetterStatus = (letter, position, guessIndex) => {
    ...
    if (guess[position] === targetWord[position]) {
      ...
      for (let i = 0; i < position; i++) {
        if (guess[i] === guess[position] && targetWord[i] !== guess[i] && targetWord.includes(guess[i])) {
          earlierOccurrences++;
        }
      }
  ...
  const renderGameBoard = () => {
          ...
          {guesses[i].split('').map((letter, j) => (
            <motion.div
              ...
              className={`w-12 h-12 sm:w-14 sm:h-14 flex items-center justify-center font-bold text-lg rounded-md
                ${getLetterStatus(letter, j, i) === 'correct' ? 'bg-emerald-600' : 
                  getLetterStatus(letter, j, i) === 'present' ? 'bg-amber-500' : 'bg-gray-700'}`}
            >
              {letter}
            </motion.div>
          ))}
        ...
        >
          {Array(5).fill(0).map((_, i) => (
            ...
            >
              ...
              {showHint && i === 2 && usedHint && !currentGuess[i] ? targetWord[2] : ''}
            </div>
          ))}
        </motion.div>
      );
    ...
    const remainingGuesses = 6 - guesses.length - (gameState === 'playing' ? 1 : 0);
    ...
    return (
      <div className="w-full max-w-md mx-auto">
        {KEYBOARD_ROWS.map((row, rowIndex) => (
          <div key={`row-${rowIndex}`} className="flex justify-center gap-1 mb-1.5">
            {row.map((key) => {
    ...
    <div className="flex flex-col items-center">
      <div className="w-full max-w-md px-2 mb-3 flex items-center justify-between">
        <Badge variant="outline" className="text-xs px-2 py-0">
          {gameState === 'playing' ? 'IN PROGRESS' : gameState === 'won' ? 'COMPLETED' : 'FAILED'}
        </Badge>
...
export default function WordlePage() {
  ...
  const handleGameComplete = (data) => {
    ...
    newStats.winPercentage = Math.round((newStats.wins / newStats.played) * 100);
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-900 to-gray-950 text-white">
      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
        ...
        <div className="text-center mb-10">
          <motion.h1 
            className="text-4xl font-extrabold mb-3 bg-clip-text text-transparent bg-gradient-to-r from-emerald-400 to-teal-500"
        ...
        <Card className="border-0 bg-gradient-to-br from-gray-800 to-gray-900 shadow-xl mb-8">
          <CardHeader className="space-y-1">
            ...
            <CardDescription>
              Find the 5-letter word in 6 tries
            </CardDescription>
          </CardHeader>
        ...
        <Card className="border-0 bg-gradient-to-r from-emerald-900/30 to-teal-900/30 shadow-md">
          ...
          <CardContent>
            <p className="text-gray-300">
              Start with a word that has common vowels and consonants like "ADIEU" or "STARE" to quickly identify which letters are in the target word.
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      <GameCompletionModal 
        ...
        isWin={currentGameData?.targetWord === currentGameData?.guesses[currentGameData?.guesses.length - 1]}
      />
