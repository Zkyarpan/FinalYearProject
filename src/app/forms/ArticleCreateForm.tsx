...
const ArticleCreateForm = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          result.ErrorMessage?.[0]?.message || 'Failed to create article'
        );
      }
      ...
      <div className="max-w-[1000px] mx-auto px-4">
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-muted-foreground mt-1">
              Share your insights and experiences to help others with their mental health journey
            </p>
          </div>
        ...
        {!isAuthenticated && (
          <div className="bg-yellow-100 dark:bg-yellow-900 p-3 rounded-md mb-6 text-yellow-800 dark:text-yellow-200">
            <p>
              You must be logged in to publish an article. Please log in first.
            </p>
          </div>
        )}
