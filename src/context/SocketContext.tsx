...
export const SocketProvider = ({ children }: { children: React.ReactNode }) => {
    ...
    async (notificationIds?: string[]) => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          // Emit event to notify socket server
  ...
  const deleteNotification = useCallback(
    async (notificationId: string) => {
      ...
      try {
        ...
        if (response.ok) {
          // Update the local count for unread notifications
          const isUnread =
            document
              .getElementById(`notification-${notificationId}`)
              ?.getAttribute('data-read') === 'false';
          if (isUnread) {
  ...
  useEffect(() => {
    ...
    socketInstance.on('new_notification', (notification: Notification) => {
      console.log('Socket: New notification received:', notification._id);

      // Increment notification count
      setNotificationCount(prev => prev + 1);

      // You could also emit an event for other parts of the app to respond to
    ...
    socketInstance.on('notification_count', (data: { count: number }) => {
      console.log('Socket: Notification count received:', data.count);
    ...
    socketInstance.on(
      ...
      (data: { success: boolean; count: number }) => {
        ...
        if (data.success) {
          // The server will tell us how many notifications were marked as read
          fetchNotificationCount();
        }
      }
    );
  ...
  }, [user?._id, user?.role, fetchNotificationCount]); // Re-run when user ID or role changes
