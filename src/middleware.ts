...
export default async function middleware(req: NextRequest) {
  try {
    ...
    if (session?.id) {
      ...
      // Handle dashboard routes - ONLY if needed
      if (path.startsWith('/dashboard')) {
        if (!session.isVerified) {
          return NextResponse.redirect(new URL('/verify', req.nextUrl));
        }
        ...
        if (session.role === 'user') {
          // For regular users, only redirect if they try to access another role's dashboard
          if (
          ...
          // For non-regular users, only redirect from base dashboard
          if (path === '/dashboard') {
            return NextResponse.redirect(new URL(dashboardPath, req.nextUrl));
          }
          // Or if they try to access a different role's dashboard
          else if (
            path.startsWith('/dashboard/') &&
            !path.startsWith(dashboardPath)
          ) {
